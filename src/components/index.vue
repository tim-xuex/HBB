...
export default {
  ...
  filters: {
    ...
    beautifyTime: function(value) {
      ...
      // 格式化日期  使用moment.js来格式化  (加载moment.js包)
      return moment(value).format("YYYY年MM月DD日");
    }
  },
  // 生命周期函数（相当于入口函数，是创建之后才有的数据）
  // 创建之前
  beforeCreate() {
    // console.log('beforeCreate');
    // console.log(this.messege);//undefined
  },
  // 创建之后 在这里面才可以访问到vue组件中的内容
  created() {
    // console.log('created');
    // console.log(this.messege);//哈哈
    axios
      .get("http://111.230.232.110:8899/site/goods/gettopdata/goods")
      .then(response => {
        // console.log(response);
        // 可以处理数据中的日期格式 再赋值（插件名：moment.js）
...
// 以前页面加载完之后干一件事原生需要用window.onload-functional(){}
// jQuery 实现页面加载完之后干一件事   $(functional(){})
// 以上这两种俗称入口函数
// **vue也有一个像这样的入口函数  名字叫做vue生命周期函数或者叫钩子函数